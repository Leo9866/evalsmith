...
import (
	...
	"github.com/evalsmith/dataset-service/internal/handler"
...
func main() {
	...
	svc := service.NewDatasetService(datasetRepo, versionRepo, exampleRepo)
	h := handler.NewHandler(svc, authServiceURL, internalToken)

	r := gin.Default()
	r.GET("/health", func(c *gin.Context) {
		c.JSON(200, gin.H{"status": "ok", "service": "dataset-service"})
	})
	h.RegisterRoutes(r)

	log.Printf("dataset-service starting on :%s", serverPort)
